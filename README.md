...
Every bmmtools function this README names is built: the five scorers of
the table, `recover_subjects()`, and the simulation, grid and cache
layers underneath them. What is not done is the release: the vignette
“Validating a new bmm model”, `cran-comments.md`, a NEWS list written
for a release rather than for development, and a
`devtools::check(cran = TRUE)` run. Until those are done the version is
0.0.0.9000 and the API can still change.
