...
`simulate_recovery()` and `recovery_grid()` need a generator for each
model. bmmtools ships adapters for a few bmm models in `R/adapters.R`;
any other model takes a `generator =` argument (see the article
"Validating a model without a built-in adapter"). To add an adapter:
...
3. If the response has a known floor and ceiling, add it to
   `response_range()` in `R/prior-check.R`, so `prior_check()` reports
   floor and ceiling rates for the model.
...
- No `set.seed()` in package code. Randomness takes a `seed` argument
  and is applied locally with `withr::with_seed()`.
...
and rebuild the site with `pkgdown::build_site()`.
